...
bool HostPortTCP::begin(std::string ipaddr, unsigned short port, unsigned int header, unsigned int terminator) {
    _terminator = terminator;
    _header = header;
    _timeout = TIMEOUT;
    _port = port;
...
bool HostPortTCP::begin(std::string ipaddr, unsigned short port, unsigned int header, unsigned int terminator, unsigned int timeout) {
    _terminator = terminator;
    _header = header;
    _port = port;
    _ip = ipaddr;
...
bool HostPortTCP::setIP(std::string ipaddr) {
   // if (!serial) {
        _ip = ipaddr;
        return true;
    //}
...
//readpy
bool HostPortTCP::readpy (boost::python::object obj) {
...
//writepy
bool HostPortTCP::writepy (boost::python::object obj) {
    PyObject* pobj = obj.ptr();
    Py_buffer pybuf;
    if(PyObject_GetBuffer(pobj, &pybuf, PyBUF_SIMPLE)!=-1){
